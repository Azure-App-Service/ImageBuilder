...
if [[ "$W3TC_CDN_CONFIGURE" == "CDN" ]] && [[ $CDN_ENDPOINT ]]; then
...
elif [[ "$W3TC_CDN_CONFIGURE" == "BLOB" ]] && [[ $BLOB_CDN_ENDPOINT ]]; then
    if wp w3-total-cache option set cdn.azure.cname $BLOB_CDN_ENDPOINT --type=array --path=$WORDPRESS_HOME --allow-root; then
        echo "CDN_CONFIGURATION_COMPLETE" >> $WORDPRESS_LOCK_FILE
        service atd stop
    else
        service atd start
